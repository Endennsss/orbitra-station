...
/// </summary>
internal sealed class LimeNightVisionOverlay : Overlay
{
    ...
    private static readonly ProtoId<ShaderPrototype> Shader = "LimeNightVision";
    private readonly LimeNightVisionPresentation _presentation;
    private readonly ShaderInstance _shader;
    private readonly ShaderInstance _exposure;
    private readonly OverlayResourceCache<ExposureResources> _resources = new();
    private static readonly ProtoId<ShaderPrototype> ExposureShader = "LimeNightVisionExposure";

    public override OverlaySpace Space => OverlaySpace.WorldSpaceBelowFOV;
    public override bool RequestScreenTexture => true;

    public LimeNightVisionOverlay(LimeNightVisionPresentation presentation)
    {
        IoCManager.InjectDependencies(this);
        _presentation = presentation;
        _shader = _prototype.Index(Shader).InstanceUnique();
        _exposure = _prototype.Index(ExposureShader).InstanceUnique();
        ZIndex = 200; // После Lime Bloom (100), до штатного FOV.
    }
    ...
    protected override void Draw(in OverlayDrawArgs args)
    {
        ...
        try
        {
            ...
            _shader.SetParameter("aperture_radii", LimeNightVisionPresentation.GetApertureRadii(size));
            _shader.SetParameter("edge_fraction", LimeNightVisionPresentation.EdgeFraction);
    ...
    private Texture DrawExposure(in OverlayDrawArgs args)
    {
        ...
        _exposure.SetParameter("adaptation", LimeNightVisionPresentation.GetAdaptationFactors(
            (float) (now - cache.LastDraw).TotalSeconds));
        _exposure.SetParameter("viewport_size", (Vector2) args.Viewport.Size);
        _exposure.SetParameter("aperture_radii", LimeNightVisionPresentation.GetApertureRadii(args.Viewport.Size));
    ...
    private sealed class ExposureResources : IDisposable
    {
        ...
        public ExposureResources(IClyde clyde)
        {
            ...
            Previous = clyde.CreateRenderTarget(Vector2i.One, format, name: "lime-nv-exposure-a");
            Next = clyde.CreateRenderTarget(Vector2i.One, format, name: "lime-nv-exposure-b");
        }
