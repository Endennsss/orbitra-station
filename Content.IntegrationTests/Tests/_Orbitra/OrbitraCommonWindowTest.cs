    ...
    public async Task GuideSectionsSwitchWithoutRebuildingTree()
    {
        ...
        foreach (var size in sizes)
        {
            ...
            await Pair.Client.WaitAssertion(() =>
            {
                ...
                Assert.That(guide.Tree.DrawLines, Is.True, "Orbitra keeps guide connectors enabled.");
                Assert.That(guide.FindControl<Content.Client.UserInterface.Controls.FancyTree.FancyTree>("TableOfContents").DrawLines, Is.True);
                if (wide)
                {
