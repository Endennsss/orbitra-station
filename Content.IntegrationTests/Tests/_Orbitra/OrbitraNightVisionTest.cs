...
[TestFixture]
public sealed class OrbitraNightVisionTest : GameTest
{
    ...
    public async Task EquipmentToggleAndViewerChangesSelectCorrectOverlay()
    {
        ...
        await server.WaitPost(() =>
        {
            ...
            goggles = server.EntMan.SpawnEntity("OrbitraClothingEyesNightVision", map.GridCoords);
        ...
        async Task AssertOverlays(bool device, bool natural)
        {
            ...
            await client.WaitAssertion(() =>
            {
                ...
                Assert.That(overlays.HasOverlay<OrbitraNightVisionOverlay>(), Is.EqualTo(device));
                Assert.That(overlays.HasOverlay<OrbitraNightVisionLightOverlay>(), Is.EqualTo(device));
