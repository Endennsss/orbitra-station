...
[TestFixture]
public sealed class OrbitraNightVisionTest
{
    [Test]
    public void ExposureHistoryResetsOnlyWhenSourceChanges()
    {
        var state = new OrbitraNightVisionPresentation();
    ...
    {
        var step = OrbitraNightVisionPresentation.GetAdaptationFactors(1f / fps);
        var residual = Vector2.One;
        for (var frame = 0; frame < fps; frame++)
            residual *= Vector2.One - step;

        var expected = Vector2.One - OrbitraNightVisionPresentation.GetAdaptationFactors(1f);
        ...
        Assert.That(OrbitraNightVisionPresentation.GetAdaptationFactors(-1f), Is.EqualTo(Vector2.Zero));
    }
    ...
    {
        var radii = OrbitraNightVisionPresentation.GetApertureRadii(new Vector2(width, height));
    ...
    public void PrioritizedWearableWinsOverNaturalVision()
    {
        ...
        Assert.That(OrbitraNightVisionPresentation.SelectSource(viewer, sources)?.Comp, Is.SameAs(device));
        device.Enabled = false;
        Assert.That(OrbitraNightVisionPresentation.SelectSource(viewer, sources)?.Comp, Is.SameAs(natural));
        natural.Enabled = false;
        Assert.That(OrbitraNightVisionPresentation.SelectSource(viewer, sources), Is.Null);
    }
    ...
    public void RelayAndNoiseRulesRemainUnchanged()
    {
        ...
        Assert.That(OrbitraNightVisionPresentation.SelectSource(viewer, sources)?.Comp, Is.SameAs(quiet));
    }
