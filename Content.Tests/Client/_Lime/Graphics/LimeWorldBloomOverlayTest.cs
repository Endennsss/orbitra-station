...
[TestFixture]
[TestOf(typeof(LimeWorldBloomOverlay))]
public sealed class LimeWorldBloomOverlayTest
{
    [Test]
    public void SourceReturnsToSameWorldPositionAfterEmissionMaskRoundTrip()
    {
        ...
        var firstTarget = LimeWorldBloomOverlay.GetSourceToTargetMatrix(
            layerMatrix,
            spriteMatrix,
            sourcePosition,
            Angle.FromDegrees(15f),
            firstWorldToTarget);
        var secondTarget = LimeWorldBloomOverlay.GetSourceToTargetMatrix(
            layerMatrix,
            spriteMatrix,
            sourcePosition,
            Angle.FromDegrees(15f),
    ...
    {
        Assert.That(LimeWorldBloomOverlay.GetBloomStrength(configured), Is.EqualTo(expected).Within(0.0001f));
    }

    [TestCase((int) LimeBloomQuality.Low, 8f, 2f)]
    [TestCase((int) LimeBloomQuality.Medium, 8f, 3f)]
    [TestCase((int) LimeBloomQuality.High, 16f, 8f)]
    [TestCase((int) LimeBloomQuality.High, 1000f, 16f)]
    [TestCase((int) LimeBloomQuality.Low, 0.5f, 2f)]
    public void BlurRadiusFitsPaddedMask(int quality, float pixelsPerMeter, float expected)
    {
        var radius = LimeWorldBloomOverlay.CalculateBlurRadius((LimeBloomQuality) quality, pixelsPerMeter);
        Assert.That(radius, Is.EqualTo(expected));
        Assert.That(radius, Is.LessThan(LimeWorldBloomOverlay.TargetPadding));
    }
    ...
    public void PaddedMaskCornersCompositeToOriginalViewport(int width, int height, int divisor, float rotation)
    {
        ...
        var mask = LimeWorldBloomOverlay.GetWorldToTargetMatrix(camera, viewportSize, contentSize);
        Assert.That(Matrix3x2.Invert(mask, out var composite), Is.True);
        var padding = new Vector2(LimeWorldBloomOverlay.TargetPadding);
    ...
    public void GridPixelSnapMatchesRoundedViewportPosition()
    {
        ...
        var offset = LimeWorldBloomOverlay.CalculatePixelSnapOffset(
            gridPosition,
            viewPosition,
            viewRotation,
            viewScale,
