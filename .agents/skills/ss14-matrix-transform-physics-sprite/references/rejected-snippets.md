...
| `_Orbitra` case with `WorldMatrix.TransformBox(LocalAABB)` for grid | Working world-AABB transition | Discarded as a duplicate of an already covered upstream pattern FTL/proximity AABB | Pattern duplication, no unique matrix idea |
