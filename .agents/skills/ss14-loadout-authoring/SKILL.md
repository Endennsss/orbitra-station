...
1. `loadout`: `LimeLoadout` + `ItemPrototypeId`.
2. `loadoutGroup`: `LimeLoadoutGroup` + slot name or generic group name.
3. `roleLoadout`: `Job<RoleId>`.
4. If the same `ItemPrototypeId` is needed in two different contexts and without the suffix there will be a collision, add a short context tail: `LimeLoadoutFlashlightLanternCargo`, `LimeLoadoutFlashlightLanternEngineering`.
...
3. Give `loadout` ID in the format `LimeLoadout<ItemPrototypeId>`.
4. Give `loadoutGroup` ID in the format `LimeLoadoutGroup<SlotOrConcept>`, and not according to the historical copy source.
...
1. Leave the `_ForkName` role to refer to a vanilla or another fork's `loadoutGroup`.
...
```yaml
- type: loadout
  id: LimeLoadoutKeyCardMedicalSpecialist
  equipment:
    id: KeyCardMedicalSpecialist

- type: loadoutGroup
  id: LimeLoadoutGroupIdKeyCardMedicalSpecialist
  name: loadout-group-id
  minLimit: 1
  loadouts:
  - LimeLoadoutKeyCardMedicalSpecialist

- type: roleLoadout
  id: JobMedicalSpecialist
  groups:
  - LimeLoadoutGroupIdKeyCardMedicalSpecialist
```
...
```yaml
- type: loadoutGroup
  id: LimeLoadoutGroupMedicalShoes
  name: loadout-group-shoes
  loadouts:
  - LimeLoadoutClothingShoesColorWhite
  - LimeLoadoutClothingShoesBootsWinterMed
...
```yaml
- type: loadoutGroup
  id: LimeLoadoutGroupSecurityBackpack
  name: loadout-group-backpack
  loadouts:
  - LimeLoadoutClothingBackpackSecurity
  - LimeLoadoutClothingBackpackSatchelSecurity
  - LimeLoadoutClothingBackpackDuffelSecurity
```
...
1. All new IDs correspond to the scheme `LimeLoadout*` / `LimeLoadoutGroup*` / `Job<RoleId>`.
