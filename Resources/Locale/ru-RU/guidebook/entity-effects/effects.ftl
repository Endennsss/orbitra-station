...
entity-effect-guidebook-electrocute =
    ...
    } метаболизатора на {NATURALFIXED($time, 3)} {MANY("секунду", $time)}
...
entity-effect-guidebook-plant-change-trait =
    { $chance ->
        [1] { $change ->
            [Add] добавляет признак {$trait}
            [Remove] удаляет признак {$trait}
            [Toggle] переключает признак {$trait}
            *[other] изменяет признак {$trait}
        }
        *[other] { $change ->
            [Add] добавляют признак {$trait}
            [Remove] удаляют признак {$trait}
            [Toggle] переключают признак {$trait}
            *[other] изменяют признак {$trait}
        }
    } растения
